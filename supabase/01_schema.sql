...
--  Orden: 1º este archivo, 2º 02_seed_matches.sql
-- ============================================================
...
-- INSERT / UPDATE / DELETE sólo desde el service role (bypassa RLS por defecto)
create policy "Service role manages matches"
  on public.matches for all
  using     (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');

-- ─── Predictions (puntuaciones de grupo) ─────────────────────────────────────
-- Fecha de cierre: primer partido → 2026-06-11 20:00 UTC
...
create policy "Predictions viewable (public profiles)"
  on public.predictions for select using (
    exists (
      select 1 from public.profiles p
      where p.id = predictions.user_id and p.is_public = true
    )
    or auth.uid() = user_id
  );

create policy "Users insert own predictions before lock"
  on public.predictions for insert with check (
    auth.uid() = user_id
    and now() < '2026-06-11 20:00:00+00'::timestamptz
  );
...
create policy "Users delete own predictions before lock"
  on public.predictions for delete using (
    auth.uid() = user_id
    and now() < '2026-06-11 20:00:00+00'::timestamptz
  );

-- ─── Group Predictions (standings calculados por grupo) ───────────────────────

create table public.group_predictions (
  id            uuid primary key default uuid_generate_v4(),
  user_id       uuid not null references public.profiles(id) on delete cascade,
  group_id      text not null,
  team_order    text[] not null,  -- [1º, 2º, 3º, 4º] team IDs
  points_earned int not null default 0,
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now(),
...
create policy "Group preds viewable"
  on public.group_predictions for select using (true);

create policy "Users manage own group preds"
  on public.group_predictions for all using (
    auth.uid() = user_id
    and now() < '2026-06-11 20:00:00+00'::timestamptz
  );
